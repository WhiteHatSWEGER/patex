...
class MQ7():
    ...
    RO_CLEAN_AIR_FACTOR = 27  # Sensor resistance in clean air divided by RO
    CALIBRATION_SAMPLE_TIMES = 50
    CALIBRATION_SAMPLE_INTERVAL = 50  # in milliseconds
    READ_SAMPLE_INTERVAL = 50  # in milliseconds
    READ_SAMPLE_TIMES = 5
    CO_CURVE = [1.499, 0.402, -0.255]  # Example curve, adjust based on actual data

    def __init__(self, Ro=10, channel=ADS.P2):
        self.Ro = Ro
        i2c = busio.I2C(board.SCL, board.SDA)
        ads = ADS.ADS1115(i2c, address=0x48)
        self.chan = AnalogIn(ads, channel)

        print("Calibrating MQ-7 on ADS1115 at address 0x48...")
        self.Ro = self.MQ_Calibration()
        print("Calibration of MQ-7 is done...")
        print("MQ-7 Ro=%f kohm" % self.Ro)
        print("\n")
    ...
    def MQ_Percentage(self):
        ...
        val["CO"] = self.MQ_GetGasPercentage(read/self.Ro, self.CO_CURVE)
...
# Example of using the MQ7 class
sensor = MQ7()
