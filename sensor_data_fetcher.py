...
# Replace the following line with the correct path to your data directory
path_to_data = "/home/htxz/Dokumente/patex/Static"

# Set up the I2C interface

  # Use 0 for Raspberry Pi version 1, 1 for Raspberry Pi version 2 and newer
...
def filter_sensor_data(from_timestamp, to_timestamp):
    sensor_data = []
    with open('static/sensor-data.json', 'r') as file:
        for line in file:
            try:
                data = json.loads(line)
                if from_timestamp <= data['Info']['TimeStamp'] <= to_timestamp:
