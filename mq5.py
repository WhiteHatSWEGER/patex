...
class MQ5():
    ...
    RO_CLEAN_AIR_FACTOR = 6.5  # Sensor resistance in clean air divided by RO
    CALIBRATION_SAMPLE_TIMES = 50
    CALIBRATION_SAMPLE_INTERVAL = 50  # in milliseconds
    READ_SAMPLE_INTERVAL = 50  # in milliseconds
    READ_SAMPLE_TIMES = 5
    LPG_CURVE = [2.3, 0.21, -0.47]  # Example curve, adjust based on actual data

    def __init__(self, Ro=10, channel=ADS.P1):
        self.Ro = Ro
        i2c = busio.I2C(board.SCL, board.SDA)
        ads = ADS.ADS1115(i2c, address=0x48)
        self.chan = AnalogIn(ads, channel)

        print("Calibrating MQ-5 on ADS1115 at address 0x48...")
        self.Ro = self.MQ_Calibration()
        print("Calibration of MQ-5 is done...")
        print("MQ-5 Ro=%f kohm" % self.Ro)
        print("\n")
    ...
    def MQ_Percentage(self):
        ...
        val["LPG"] = self.MQ_GetGasPercentage(read/self.Ro, self.LPG_CURVE)
...
# Example of using the MQ5 class
sensor = MQ5()
