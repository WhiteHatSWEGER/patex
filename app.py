import os
from flask import Flask, render_template, jsonify, request
import json
import time
import requests
from Api import Api
from flasgger import Swagger

from sensor_data_fetcher import filter_sensor_data     

app = Flask(__name__)
swagger = Swagger(app)
app.config['SWAGGER'] = { 'title' : 'WING ToDoes API' }
api = Api("static/sensor-data.json") 


# ...





@app.route('/api/sensors')
def get_sensors():
    api = Api("static/sensor-data.json")  # Replace with the path to your sensor data
    return jsonify(api.get_sensors())

@app.route('/api/sensor-data', methods=['POST'])
def add_sensor_data():
    api = Api("static/sensor-data.json")  # Replace with the path to your sensor data
    data = request.get_json()
    sensor_id = data.get('sensor_id')
    sensor_data = data.get('sensor_data')
    return jsonify(api.add_sensor_data(sensor_id, sensor_data))
def send_sensor_data():
    if os.environ.get('FLASK_ENV') == 'production':

        response = requests.post("http://127.0.0.1:5000/sensor-data", json=sensor_data)

        if response.status_code != 200:

            print("Failed to send sensor data")
        else:  
            sensor_data = {
                "id": 1,
                "timestamp": int(time.time()),
                "sensor_values": [
                    {
                        "sensor_name": "Sensor1",
                        "value": 10,
                        "unit": "unit1"
                    },
                    # ... Add more sensor data here
                ]
            }
            response = requests.post(
                "http://127.0.0.1:5000/sensor-data",  # Update the IP address and port if needed
                json=sensor_data
            )
            if response.status_code == 200:
                print("Sensor data sent successfully")
            else:
                print(f"Error sending sensor data: {response.status_code} - {response.text}")

@app.route('/')
def home():
    with open('static/sensor-data.json') as f:
        sensor_data = json.load(f)
    return render_template('index.html', sensor_data=sensor_data)

@app.route('/data')
def get_data():
    with open('static/sensor-data.json') as f:
        sensor_data = json.load(f)
    return json.dumps(sensor_data)


def handle_sensor_data():
    data = request.get_json()
    id = data.get('id')
    timestamp = data.get('timestamp')
    sensor_values = data.get('sensor_values', [])

    # Process the sensor data here
    # Store it in a database or use it as needed

    return jsonify({'message': 'Sensor data received'})
    
@app.route('/api/sensor-data')
def get_sensor_data():
    from_timestamp = request.args.get('from')
    to_timestamp = request.args.get('to')

    if from_timestamp and to_timestamp:
        from_timestamp = int(from_timestamp)
        to_timestamp = int(to_timestamp)

        # Filter the sensor data based on the provided timestamps
        # Replace the following line with the actual filtering logic
        sensor_data = filter_sensor_data(from_timestamp, to_timestamp)
        return jsonify(sensor_data)
    return jsonify({'error': 'Invalid request'})

@app.route('/api/event-data')
def get_event_data():
    event_data = [
        {
            'timestamp': '2023-03-10 10:00:00',
            'message': 'System 1 started.',
            'level': 'info'
        },
        {
            'timestamp': '2023-03-10 11:30:00',
            'message': 'System 2 temperature is high.',
            'level': 'warning'
        },
        {
            'timestamp': '2023-03-10 14:45:00',
            'message': 'System 3 is offline.',
            'level': 'error'
        }
    ]
    return jsonify(event_data)
    
if __name__ == '__main__':
    app.run(debug=True, port = 5000)
